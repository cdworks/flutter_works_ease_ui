...
class ChatRetransmissionContactPageScaffold extends CupertinoPageScaffold {
  ...
  ObstructingPreferredSizeWidget get navigationBar => WorksCupertinoNavigationBar(
    border: null,
    middle: FittedBox(
        fit: BoxFit.contain,child:Text('选择转发对象',style: TextStyle(color: titleColor),)),
  );


  const ChatRetransmissionContactPageScaffold(
      this.delegate,
      this.message,
...
class ChatRetransmissionContactPage extends StatefulWidget {

  final ChatDelegate delegate;
  final String currentUserId;  //当前用户id
  final EMMessage message;
  final String currentConversation;
  const ChatRetransmissionContactPage(
      this.delegate,
      this.message,
      this.currentConversation,
  ...
  @override
  State<StatefulWidget> createState() => _ChatRetransmissionContactPage();
}

class _ChatRetransmissionContactPage extends State<ChatRetransmissionContactPage> {
            ...
            itemBuilder: (BuildContext context, int index) {
              ...
              if (index > contactIndex) {
                ...
                return _ContactItemCell(
                  userModel,
                  tapCallback: () {
                      toSendMessage(userModel.userId, ChatType.Chat,userModel.nickname);
                  },
                );
              } else {
                ...
                return _ContactItemCell(
                  groupModel,
                  tapCallback: () {
                    toSendMessage(groupModel.getGroupId(), ChatType.GroupChat,groupModel.getGroupName());
                  },
                );
              }
            },
  ...
  void toSendMessage(String conversationId,ChatType type,String name)
  {
    var msgType = widget.message.type;

    EMMessage sendMessage;
    if(msgType == EMMessageType.TXT )
    {
      var body = widget.message.body as EMTextMessageBody;
      sendMessage = EMMessage.createTxtSendMessage(body.message,conversationId);

    }
    else if(msgType == EMMessageType.FILE )
    {
      var body = widget.message.body as EMNormalFileMessageBody;
      sendMessage = EMMessage.createFileSendMessage(body.localUrl, conversationId,body.fileSize);
      var msgBody = sendMessage.body as EMNormalFileMessageBody;
      msgBody.displayName = body.displayName;

    }
    else if(msgType == EMMessageType.VIDEO)
      {
        var body = widget.message.body as EMVideoMessageBody;
        sendMessage = EMMessage.createVideoSendMessage(body.localUrl, body.videoDuration, conversationId, body.thumbnailLocalPath??body.thumbnailRemotePath??'' , body.thumbnailWidth, body.thumbnailHeight);
      }
    else if(msgType == EMMessageType.IMAGE )
    {
      var body = widget.message.body as EMImageMessageBody;
      sendMessage = EMMessage.createImageSendMessage(body.localUrl, true,conversationId);
      var msgBody = sendMessage.body as EMImageMessageBody;
      msgBody.setThumbnailSize(body.width, body.height);

    }
    else if(msgType == EMMessageType.LOCATION )
    {
      var body = widget.message.body as EMLocationMessageBody;
      sendMessage = EMMessage.createLocationSendMessage(body.latitude, body.longitude,body.address,conversationId);
    }

    if(sendMessage != null)
      {
        sendMessage.status = Status.CREATE;

        sendMessage.msgTime = DateTime.now().millisecondsSinceEpoch.toString();
        sendMessage.localTime = sendMessage.msgTime;
        sendMessage.chatType = type;
      }
    else
      {
        Toast.show("文件类型错误，无法转发!", context,gravity: Toast.CENTER,backgroundRadius: 8,duration:Toast.LENGTH_LONG);
        return;
      }
              ...
              CupertinoDialogAction(
                ...
                onPressed: () {
                  ...
                  EMClient.getInstance().chatManager().sendMessage(sendMessage,
                      onSuccess: () {
                        Toast.show("消息转发成功!", context,gravity: Toast.CENTER,backgroundRadius: 8,duration:Toast.LENGTH_LONG);

                        if(conversationId == widget.currentConversation)
                          {
                        ...
                        Toast.show("消息转发失败：$desc!", context,gravity: Toast.CENTER,backgroundRadius: 8);
                      });
                },
                child: Text('确定'),
              ),
